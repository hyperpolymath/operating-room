...
----
           ...
           ▼                        ▼                        ▼
┌──────────────────┐    ┌──────────────────┐    ┌──────────────────┐
│ system-emergency │    │ system-operating │    │      jusys       │
...
=== CRIT-001: GenServer Race Condition in JuSys Correlator

*Location:* `jusys/lib/jusys/correlator.ex`
...
*Location:* `jusys/lib/jusys/metrics/store.ex`
...
*Violation:* JuSys stores derived state that other systems query as authoritative:
...
4. Document clearly that all JuSys data is advisory
...
*Description:* If JuSys is down, other systems have no fallback behavior.
...
|CRIT-003
|Add provenance tracking to JuSys
|JuSys
...
Deno.test("System degrades gracefully when JuSys unavailable", async () => {
  await chaos.killService("jusys");
  const incident = await emergencyRoom.createIncident();
...
1. **Fix race conditions** in JuSys correlator
